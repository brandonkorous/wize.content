            ...
            foreach (var description in provider.ApiVersionDescriptions)
            {
                options.SwaggerDoc(
                  description.GroupName,
                    new OpenApiInfo()
                    {
                        Title = $"wize.content.odata API {description.ApiVersion}",
                        Version = description.ApiVersion.ToString(),
                        Description = "OData API for content." + (description.IsDeprecated ? " This API Version is deprecated." : string.Empty),
