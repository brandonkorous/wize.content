...
{
    public class ContentModelConfiguration : IModelConfiguration
    {
        ...
        private EntityTypeConfiguration<Content> BuildDefault(ODataModelBuilder builder)
        {
            var model = builder.EntitySet<Content>("Contents").EntityType;
            //model.HasKey(m => m.ContentId);
            //model.Filter().Count().Expand().OrderBy().Page().Select();
            return model;
        }
